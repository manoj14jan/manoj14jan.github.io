...
public class FormValidation extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(String error:errors) {
			if(error!=null) {
				request.getRequestDispatcher("/contact-us-form.jsp").forward(request, response);
				return;
			}
		}

		response.sendRedirect("http://localhost:8080/w3d3/thank-you.jsp?name="+name+"&gender="+gender+"&category="+category+"&message="+message);
